...
```
*The website will be available at `https://localhost:8443` and the interactive API at `https://localhost:8443/swagger-ui.html`.*
...
⚠️ *You must be connected to the URJC network (VPN/MyApps).*
...
*The production application will be available at `https://appweb05.dawgis.etsii.urjc.es:8443`.*
...
* **Entry and Routing:** `main.tsx` and `root.tsx` work as the main core, sending navigation to the different views through *Outlets*.
...
* **Modules and UI:** Each main section has its own nested sub-routes and uses reusable interface components (such as *Headers*, *Footers*, and *Sidebars*).
* **State and Backend:** *Zustand* (`useUserStore.ts`) manages the user session globally, while the *Services* layer centralizes the HTTP logic, using `api.ts` to connect smoothly with the Spring Boot REST backend.
